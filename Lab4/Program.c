...
// pass each thread a pointer to its respective argument 
    if ((status = pthread_create (&thread1, NULL,  file_accesser, &val[0])) != 0) { 
...
// print out a message
   ...
   printf("Please input the file you want to access: ");
   fgets(file,5000,stdin);
